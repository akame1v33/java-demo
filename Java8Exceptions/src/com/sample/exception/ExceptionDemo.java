...
public class ExceptionDemo implements  Runnable{
    ...
    static public void testUnChecked()  {
        boolean x = true;
        if( x  ){
            throw new ExceptionDemo.TestExceptionHandler.UncheckedExceptionDemo("FUCK WTF IS HAPPENING");
        } else {
            System.out.println("WTF?");
        }
    }
    ...
    public void run() {
        ...
        try {
            testChecked();
        } catch (ExceptionDemo.TestExceptionHandler.CheckedExceptionDemo checkedExceptionDemo) {
